...
public class MovementManager {
    ...
    /**
     * This method is used by the PhysicsEngine to check for wall collisions.
     * It tells the engine exactly where the player is TRYING to go.
     */
    ...
    private void handlePhysicsMovement(Vector3f dir, Vector3f leftVec, float tpf) {
        ...
        if (forward) lastMoveDir.addLocal(walkDir);
        if (back)    lastMoveDir.subtractLocal(walkDir);
        if (left)    lastMoveDir.addLocal(walkLeft);
        if (right)   lastMoveDir.subtractLocal(walkLeft);
        ...
        // Apply the final movement to the player data
        player.position.addLocal(lastMoveDir.mult(speed));
    }
