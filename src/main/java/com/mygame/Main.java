...
public class Main extends SimpleApplication {
    ...
    public void onAction(String name, boolean isPressed, float tpf) {
        ...
        // 2. Raycast Actions (Place and Delete)
        // We only trigger these when the key is PRESSED (isPressed == true)
        else if (name.equals("Shoot") && isPressed) {
            raycastManager.placeBlock(); // Replaces the old Ray/Box/Geometry code
        } 
        else if (name.equals("Delete") && isPressed) {
            raycastManager.deleteBlock(); // Replaces the old target.removeFromParent code
        } 
        
        // 3. Other Player Actions
    ...
    public void simpleInitApp() {
        player = new Player();
        movementManager = new MovementManager(player); // LINKED HERE
        ...
        // Add Listeners
        inputManager.addListener(actionListener, "Forward", "Back", "Left", "Right", "ToggleGhost", "Shoot", "Delete");
